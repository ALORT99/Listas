...
int main(){
	...
	int numCaracteres;//Variable que almacenara el numero de 
caracteres que se van a ingresar
	char *caracter;//Variable para crear arreglo de caracteres 
dinamico
	char auxCaracter;//Variable que guardara los caracteres del 
arreglo y para pasar los caracteres a las funciones 
	char auxC;//Variable que servira como auxiliar para eliminar el 
caracter de la pila
		...
		switch(opcion){
			case '1':
				cout<<"\n1.Insertar datos."<<endl
				    <<"\nCuantos caracteres desea 
ingresar? "; cin>>numCaracteres;
				//fflush(stdin);
				caracter = new char[numCaracteres];
				cout<<"\nIngresar expresion matematica: 
"; cin.getline(caracter,numCaracteres+10,'\n');//+50 para que el 
programa no se cicle si se ingresan mas caracteres 
				//fflush(stdin);
...
inPos(frente,fin,auxCaracter);
				...
				//Si pila diferente de NULL hay datos en 
la pila 
				while(pila !=NULL){
					
vaciarPila(pila,frente,fin);//Vaciando la pila y insertantodos en la 
cola
				}
				break;
			case '2':
...
//Cuando el ciclo for termina manda todos los caracteres de la pila a la 
cola 
